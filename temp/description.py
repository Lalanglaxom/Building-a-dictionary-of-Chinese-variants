...
def format_text_with_images(text, section_type="default", extra_css="", font_stack=DEFAULT_FONTS):
    """
    Formats text with images and injects specific CSS for rare characters.
    """
    if not text: return ""
    ...
    # We inject the extra_css (font-face) and use the font_stack
    return f"""
    <html>
    <head>
        <style>
...
class VariantCharacterBox(QFrame):
    """
    The Variant Box in the list (already working).
    """
...
# ═══════════════════════════════════════════════════════════════════════════
# 5. UI COMPONENTS (Table Rows, Detail Window)
# ═══════════════════════════════════════════════════════════════════════════
...
class TableRow(QFrame):
    ...
    def set_html_content(self, html, min_height=60):
        ...
        # Note: formatting happens before calling this
...
class VariantDetailWindow(QDialog):
    ...
    def initUI(self):
        ...
        if details:
            ...
            # --- Row 1: Header (Code + Large Character) ---
            row1 = TableRow("Variant\nCharacter")
            h_box = QWidget(); h_lay = QHBoxLayout(); h_lay.setContentsMargins(0,0,0,0); h_lay.setSpacing(20)
            
            # Code Label
            code_label = QLabel(f"[{variant_code}]"); code_label.setStyleSheet(f"font-size:24px;font-weight:bold;color:#555;"); h_lay.addWidget(code_label)
            
            # Glyph Image (if available)
            resolved_glyph = resolve_image_path(glyph_path) if glyph_path else None
            if resolved_glyph:
                glyph_label = QLabel(); glyph_label.setPixmap(QPixmap(resolved_glyph).scaled(64, 64, Qt.KeepAspectRatio, Qt.SmoothTransformation)); h_lay.addWidget(glyph_label)
            
            # *** FIXED: Large Variant Character Display ***
            if variant_char:
                char_view = QWebEngineView()
                # Increase size to avoid scrollbars
                char_view.setFixedSize(150, 100)
                char_view.page().setBackgroundColor(Qt.transparent)
                # Ignore mouse events so it feels like a label
                char_view.setAttribute(Qt.WA_TransparentForMouseEvents)
                
                # CSS: overflow:hidden prevents scrollbars. Flexbox centers it.
                char_html = f"""
                <html>
                <head>
                    <style>
