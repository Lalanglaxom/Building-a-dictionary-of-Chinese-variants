...
# Try to import fontTools for "Anti-Tofu" detection
try:
    from fontTools.ttLib import TTFont
    HAS_FONTTOOLS = True
except ImportError:
    HAS_FONTTOOLS = False
    print("WARNING: 'fonttools' library not found. Tofu detection will be disabled.")
    print("Run 'pip install fonttools' for exact character matching.")
...
class FontManager:
    def __init__(self):
        self.css_rules = []
        self.font_families = []
        self.char_map = {} # Maps Unicode Integer -> Filename
        self.scan_fonts()

    def scan_fonts(self):
        """
        Scans fonts to build the CSS list AND (if fonttools exists) the Character Map.
        """
        ...
        for filename in files:
            ...
            # 1. Build CSS Rule (for the "Try All" list)
            ...
            # 2. Build Character Map (The "Is it Tofu?" check)
            if HAS_FONTTOOLS:
                ...
                try:
                    ...
                    if cmap:
                        for char_code in cmap:
                            # If we haven't found a font for this char yet, store it
    ...
    def get_font_data_for_char(self, char_str):
        """
        Returns (CSS, FontFamily) optimized for a specific character.
        """
        if not char_str: return "", DEFAULT_FONTS

        # STRATEGY 1: Exact Match (Requires fonttools) - 100% Accurate, No Tofu
        if HAS_FONTTOOLS:
            code_point = ord(char_str[0])
            if code_point in self.char_map:
                data = self.char_map[code_point]
                # We found the EXACT font. Only load this one!
                css = f"@font-face {{ font-family: '{data['family']}'; src: url('file:///{data['path']}'); }}"
                return css, f"'{data['family']}'"
            else:
                # We know for a fact we don't have it.
                return "", DEFAULT_FONTS

        # STRATEGY 2: "Pray and Spray" (No fonttools) - Dumps all fonts into CSS
        # This relies on the browser trying them one by one.
        full_css = "".join(self.css_rules)
        full_stack = ", ".join(self.font_families)
...
def format_text_with_images(text, section_type="default"):
    # Standard HTML formatter - uses System Fonts (No custom CSS injection)
    if not text: return ""
    ...
    text = text.replace('\n', '<br>')
    return f'<html><body style="font-family:{DEFAULT_FONTS};font-size:14px;line-height:1.9;margin:0;padding:5px;">{text}</body></html>'
...
class VariantCharacterBox(QFrame):
    """
    The ONLY widget that applies the heavy Font CSS logic.
    """
    ...
    def __init__(self, char="", code="", img_path="", variant_code="", parent=None):
        ...
        if resolved_img_path:
            ...
            self.is_image = False
            # Use QWebEngineView for CSS injection
    ...
    def render_char_html(self, text_color="#333"):
        ...
        # Ask FontManager for the specific CSS for THIS character
        ...
        <body>
            <div class="char">{char_to_show}</div>
        </body>
        </html>
        """
...
# ═══════════════════════════════════════════════════════════════════════════
# 5. UI COMPONENTS (Reverted to Standard Fonts)
# ═══════════════════════════════════════════════════════════════════════════
...
class SectionHeader(QFrame):
    def __init__(self, text, parent=None):
        ...
        accent = QFrame(); accent.setFixedSize(4, 20); accent.setStyleSheet("background:white;border:none;")
        layout.addWidget(accent)
        label = QLabel(text); label.setStyleSheet("color:white;font-size:14px;font-weight:bold;background:transparent;border:none;")
        layout.addWidget(label); layout.addStretch()
        self.setLayout(layout); self.setStyleSheet("QFrame{background:#8B0000;border:none;}")
...
class TableRow(QFrame):
    def __init__(self, label_text, has_top_accent=False, parent=None):
        ...
        if has_top_accent:
            accent = QFrame(); accent.setFixedHeight(3); accent.setStyleSheet("background:#8B0000;border:none;")
            main_layout.addWidget(accent)
        row_widget = QWidget()
        layout = QHBoxLayout(); layout.setContentsMargins(0, 0, 0, 0); layout.setSpacing(0)
        
        self.label_frame = QFrame(); self.label_frame.setFixedWidth(100)
        self.label_frame.setStyleSheet("QFrame{background:#F5F5F0;border:1px solid #CCC;border-right:none;}")
        l_layout = QVBoxLayout(); l_layout.setContentsMargins(8, 8, 8, 8)
        label = QLabel(label_text); label.setAlignment(Qt.AlignCenter); label.setStyleSheet("font-size:13px;font-weight:bold;color:#333;"); label.setWordWrap(True)
        l_layout.addWidget(label); self.label_frame.setLayout(l_layout); layout.addWidget(self.label_frame)
        
        self.content_frame = QFrame(); self.content_frame.setStyleSheet("QFrame{background:white;border:1px solid #CCC;}")
        self.content_layout = QVBoxLayout(); self.content_layout.setContentsMargins(10, 8, 10, 8)
        self.content_frame.setLayout(self.content_layout); layout.addWidget(self.content_frame, 1)
        
        row_widget.setLayout(layout); main_layout.addWidget(row_widget); self.setLayout(main_layout)
    ...
    def set_html_content(self, html, min_height=60):
        ...
        text_edit.setStyleSheet(f"QTextEdit{{font-family:{DEFAULT_FONTS};font-size:14px;color:#333;background:white;border:none;}}")
...
class VariantDetailWindow(QDialog):
    ...
    def initUI(self):
        ...
        content_widget = QWidget(); content_widget.setStyleSheet("background-color: #E8D4C8;")
        ...
        if details:
            ...
            # Row 1: Basic Info (Normal Fonts for Text, Image for Glyph)
            row1 = TableRow("Variant\nCharacter")
            h_box = QWidget(); h_lay = QHBoxLayout(); h_lay.setContentsMargins(0,0,0,0)
            code_label = QLabel(f"[{variant_code}]"); code_label.setStyleSheet(f"font-size:16px;color:#333;"); h_lay.addWidget(code_label)
            ...
            # Note: We display the Char here using standard font. 
            # If you want the custom font here too, you'd need another WebEngineView, but user said "only variants box".
            if variant_char:
                char_label = QLabel(variant_char); char_label.setStyleSheet(f"font-size:20px;color:#333;"); h_lay.addWidget(char_label)
            ...
            if key_refs: row2.set_html_content(format_text_with_images(key_refs, "default"), 80)
            else: row2.add_text_label("No data available")
            ...
            if any([bopomofo, pinyin, researcher, explanation]):
                ...
                if explanation: r = TableRow("Content", has_top_accent=True); r.set_html_content(format_text_with_images(explanation, "default"), 150); content_layout.addWidget(r)
        
        btn_frame = QFrame(); btn_layout = QHBoxLayout(); btn_layout.setContentsMargins(20, 15, 20, 15); btn_layout.addStretch()
        close_btn = QPushButton("Close"); close_btn.setStyleSheet("QPushButton{background:#666;color:white;border:none;border-radius:5px;padding:8px 25px;}"); close_btn.clicked.connect(self.close)
        btn_layout.addWidget(close_btn); btn_frame.setLayout(btn_layout); content_layout.addWidget(btn_frame)
...
class CharacterDictionary(QMainWindow):
    ...
    def initUI(self):
        ...
        scroll = QScrollArea(); scroll.setWidgetResizable(True); scroll.setStyleSheet("QScrollArea{border:none;background-color:#E8D4C8;}")
        content_widget = QWidget(); content_widget.setStyleSheet("background-color: #E8D4C8;")
        content_layout = QVBoxLayout(); content_layout.setContentsMargins(20, 20, 20, 20); content_layout.setSpacing(0)
        
        # Header (Standard Fonts for Main Char)
        ...
        # Main Char Label (Reverted to Normal QLabel)
        self.main_char_label = QLabel(""); self.main_char_label.setStyleSheet(f"font-size:100px;color:#8B0000;font-weight:bold;font-family:{DEFAULT_FONTS};"); h_layout.addWidget(self.main_char_label)
        ...
        self.entry = QLineEdit(); self.entry.setPlaceholderText("Type char..."); self.entry.setMaxLength(2); self.entry.setFixedWidth(150); self.entry.setStyleSheet(f"font-size:24px;padding:5px;border:2px solid #8B0000;border-radius:5px;font-family:{DEFAULT_FONTS};"); self.entry.returnPressed.connect(self.search_character); i_layout.addWidget(self.entry)
        ...
        self.variants_scroll = QScrollArea(); self.variants_scroll.setWidgetResizable(True); self.variants_scroll.setFixedHeight(120); self.variants_scroll.setStyleSheet("QScrollArea{border:none;background:transparent;}")
    ...
    def update_variants_display(self, char):
        ...
        if not variants:
            no_variants_label = QLabel("No variants found"); no_variants_label.setStyleSheet("color:#999;font-size:14px;padding:10px;"); self.variants_layout.addWidget(no_variants_label); self.variants_layout.addStretch(); return
        
        for variant_code, variant_char, img_path in variants:
            suffix = f".{variant_code.split('-')[-1]}" if variant_code and '-' in variant_code else ""
            # Pass data to VariantCharacterBox (Logic is inside the box class)
    ...
    def clear_all(self):
        self.entry.clear(); self.code_label.setText(""); self.main_char_label.setText(""); self.stroke_label.setText("")
        for row in self.rows: row.clear_content()
